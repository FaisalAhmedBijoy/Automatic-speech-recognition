...
config = Config()
ENGLISH_ASR_MODEL = config.ENGLISH_ASR_MODEL
...
transcriber = pipeline(HUGGINGFACE_PIPELINE, model=ENGLISH_ASR_MODEL, device=0 if torch.cuda.is_available() else -1)
...
@router.get("/")
async def get_index():  
    return {"message": "Welcome to the English Speech Recognition API!"}

@router.websocket("/en_audio")
async def process_audio(websocket: WebSocket):
    ...
    try:
        while WebSocketState.CONNECTED:
            ...
            transcription = transcriber({"sampling_rate": sample_rate, "raw": reduced_noise_audio}, return_timestamps=True)["text"]
